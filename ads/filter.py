...
from app import APPBase, Mihomo
...
class Filter(object):
    ...
    def generate(self, sourceRule):
        ...
        # 生成合并规则（仅保留 Clash Meta(Mihomo) 格式）
        generaterList:List[APPBase] = [
            Mihomo            (blockList, unblockList, filterDict, filterList, filterList_var, ChinaSet, self.path + "/jiekouAD.yaml",                  sourceRule),
        ]
