...
- 其余规则集（`category-ads-all`、`icloud`、`apple-cn`、`google-cn` 等）见 [`meta分支`](https://github.com/haha12358/clash-rules/tree/meta)：`geosite/`、`geoip/`、`asn/` 为规则集目录，`other/` 内含 `applications`、`fakeip-filter` 与 `hagezi-pro`（源自 [hagezi dns-blocklists](https://github.com/hagezi/dns-blocklists) PRO 版，广告/追踪域名拦截）。
